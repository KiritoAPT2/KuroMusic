...
object NewPipeUtils {
     ...
     */
    suspend fun getStreamUrl(format: PlayerResponse.StreamingData.Format, videoId: String): Result<String> =
        runCatching {
            require(videoId.isNotBlank()) { "Video ID cannot be blank" }
            ...
            format.signatureCipher?.let { signatureCipher ->
                ...
                val sigDecrypted = decryptCiphers(emptyList(), listOf(obfuscatedSignature), player)
                ...
                if (nParam != null && nParam.isNotBlank()) {
                    ...
                    val nDecrypted = decryptCiphers(listOf(nParam), emptyList(), player)
